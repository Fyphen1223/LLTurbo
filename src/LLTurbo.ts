...
import { LLTurboDefaults, VoiceState } from './Constants';
...
export interface LLTurboOptions {
	/**
	...
	userAgent?: string;
	/**
	 * Custom structures for LLTurbo to use
	 */
...
export declare interface LLTurbo {
	/**
	...
	/**
	 * Emitted when LLTurbo is ready to receive operations
	 * @eventProperty
	 */
 ...
 * Main LLTurbo class
 */
export class LLTurbo extends EventEmitter {
	/**
	 ...
	 * LLTurbo options
	 */
	public readonly options: Required<LLTurboOptions>;
	/**
	 ...
	 * @param options Options to pass to create this LLTurbo instance
	 ...
	 * @param options.structures Custom structures for LLTurbo to use
	 * @param options.nodeResolver Used if you have custom lavalink node resolving
	 */
	constructor(connector: Connector, nodes: NodeOption[], options: LLTurboOptions = {}) {
		super();
		this.connector = connector.set(this);
		this.options = mergeDefault(LLTurboDefaults, options);
		this.nodes = new Map();
	 ...
	 * @param args Additional arguments for LLTurbo to emit
